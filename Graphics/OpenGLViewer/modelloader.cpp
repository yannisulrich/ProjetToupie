...
void ModelLoader::getBuffer(QVector<float> **vertices, QVector<float> **normals, QVector<unsigned int> **indices)
{
    if(vertices != 0)
        *vertices = &m_vertices;
...
void ModelLoader::getTexture(QVector<QVector<float> > **textureUV, QVector<float> **tangents, QVector<float> **bitangents)
{
    if(tangents != 0)
        *tangents = &m_tangents;
    if(textureUV != 0)
